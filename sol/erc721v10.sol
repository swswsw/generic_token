 ...
 * 
 * v1.0 change - add tokenMetadata, add token name and symbol
 */
contract Erc721 {
  //using SafeMath for uint256;
  
  string constant private tokenName = "fan coin";
  string constant private tokenSymbol = "fan1.0";
  ...
  // mapping from token id to metadata of that token
  ...
  */
  ...
  /**
   * @dev returns metadata of a given token ID
   * @param _tokenId uint256 id of the token to query the metadata of
   * @return string metadata information.  an url that points to where metadata is stored.
   */ 
  ...
  */
  ...
  /**
   * @dev associate the metadata with the token
   * @param _tokenId uint256 ID of the token that the metadata will be associated with
   * @param metadata string an url that points to where metadata is stored
   */
  function setMetadata(uint256 _tokenId, string metadata) public onlyOwnerOf(_tokenId) {
